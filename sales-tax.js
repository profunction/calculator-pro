...
(function () {
  ...
  function calculate() {
    (function() { var amount = parseFloat(document.getElementById('purchaseAmount').value) || 0; var taxRate = parseFloat(document.getElementById('taxRate').value) || 0; var taxAmount = amount * (taxRate / 100); var totalPrice = amount + taxAmount; return { taxAmount: Math.round(taxAmount * 100) / 100, totalPrice: Math.round(totalPrice * 100) / 100, taxRate: taxRate }; })()
  }

  function updateKPIs(result) {
    document.getElementById('kpi_taxAmount').textContent = fmtCurrency(result.taxAmount);
document.getElementById('kpi_totalPrice').textContent = fmtCurrency(result.totalPrice);
document.getElementById('kpi_taxRate').textContent = result.taxRate || '--';
