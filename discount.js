...
(function () {
  ...
  function calculate() {
    (function() { var original = parseFloat(document.getElementById('originalPrice').value) || 0; var percent = parseFloat(document.getElementById('discountPercent').value) || 0; var discountAmount = original * (percent / 100); var finalPrice = original - discountAmount; var savingsPercent = percent; return { discountAmount: Math.round(discountAmount * 100) / 100, finalPrice: Math.round(finalPrice * 100) / 100, savingsPercent: Math.round(savingsPercent * 100) / 100 }; })()
  }

  function updateKPIs(result) {
    document.getElementById('kpi_discountAmount').textContent = fmtCurrency(result.discountAmount);
document.getElementById('kpi_finalPrice').textContent = fmtCurrency(result.finalPrice);
document.getElementById('kpi_savingsPercent').textContent = result.savingsPercent || '--';
