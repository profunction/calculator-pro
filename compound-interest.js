/* =====================================================
   Calculator Pro -- compound-interest.js
...
(function () {
  ...
  function calculate() {
    (function() { var principal = parseFloat(document.getElementById('principal').value) || 0; var rate = parseFloat(document.getElementById('rate').value) || 0; var years = parseFloat(document.getElementById('years').value) || 0; var frequency = parseInt(document.getElementById('frequency').value) || 1; var r = rate / 100; var n = frequency; var totalAmount = principal * Math.pow(1 + r / n, n * years); var totalInterest = totalAmount - principal; var ear = (Math.pow(1 + r / n, n) - 1) * 100; return { totalAmount: Math.round(totalAmount * 100) / 100, totalInterest: Math.round(totalInterest * 100) / 100, effectiveAnnualRate: Math.round(ear * 100) / 100 }; })()
  }

  function updateKPIs(result) {
    document.getElementById('kpi_totalAmount').textContent = fmtCurrency(result.totalAmount);
document.getElementById('kpi_totalInterest').textContent = fmtCurrency(result.totalInterest);
document.getElementById('kpi_effectiveAnnualRate').textContent = result.effectiveAnnualRate || '--';
