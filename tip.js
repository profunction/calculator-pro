...
(function () {
  ...
  function calculate() {
    (function() { var bill = parseFloat(document.getElementById('billAmount').value) || 0; var tipPct = parseFloat(document.getElementById('tipPercent').value) || 0; var people = parseInt(document.getElementById('numPeople').value) || 1; var tipAmount = bill * (tipPct / 100); var totalBill = bill + tipAmount; var perPerson = totalBill / people; return { tipAmount: Math.round(tipAmount * 100) / 100, totalBill: Math.round(totalBill * 100) / 100, perPerson: Math.round(perPerson * 100) / 100 }; })()
  }

  function updateKPIs(result) {
    document.getElementById('kpi_tipAmount').textContent = fmtCurrency(result.tipAmount);
document.getElementById('kpi_totalBill').textContent = fmtCurrency(result.totalBill);
document.getElementById('kpi_perPerson').textContent = fmtCurrency(result.perPerson);
