...
(function () {
  ...
  function calculate() {
    (function() { var unit = document.getElementById('unitSystem').value; var weight = parseFloat(document.getElementById('weight').value) || 0; var height = parseFloat(document.getElementById('height').value) || 0; var bmi; if (unit === 'metric') { var heightM = height / 100; bmi = weight / (heightM * heightM); } else { bmi = (weight / (height * height)) * 703; } var category; if (bmi < 18.5) category = 'Underweight'; else if (bmi < 25) category = 'Normal Weight'; else if (bmi < 30) category = 'Overweight'; else category = 'Obese'; var healthyMin = 18.5; var healthyMax = 24.9; return { bmi: Math.round(bmi * 10) / 10, category: category, healthyWeightMin: healthyMin, healthyWeightMax: healthyMax }; })()
  }

  function updateKPIs(result) {
    document.getElementById('kpi_bmi').textContent = fmtCurrency(result.bmi);
document.getElementById('kpi_category').textContent = result.category || '--';
document.getElementById('kpi_healthyWeightMin').textContent = result.healthyWeightMin || '--';
