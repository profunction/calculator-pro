/* =====================================================
   Calculator Pro -- down-payment.js
   Down Payment Calculator -- Calculation Logic
   ===================================================== */
(function () {
  "use strict";
  ...
  function calculate() {
    (function() { var price = parseFloat(document.getElementById('homePrice').value) || 0; var dpPct = parseFloat(document.getElementById('downPaymentPercent').value) || 0; var termYears = parseInt(document.getElementById('loanTerm').value) || 30; var rate = parseFloat(document.getElementById('interestRate').value) || 0; var downPayment = price * (dpPct / 100); var loanAmount = price - downPayment; var monthlyRate = rate / 100 / 12; var n = termYears * 12; var monthlyPayment; if (monthlyRate === 0) { monthlyPayment = loanAmount / n; } else { monthlyPayment = loanAmount * (monthlyRate * Math.pow(1 + monthlyRate, n)) / (Math.pow(1 + monthlyRate, n) - 1); } var totalCost = monthlyPayment * n + downPayment; var pmi = dpPct < 20 ? (loanAmount * 0.0055 / 12) : 0; return { downPaymentAmount: Math.round(downPayment * 100) / 100, loanAmount: Math.round(loanAmount * 100) / 100, monthlyPayment: Math.round((monthlyPayment + pmi) * 100) / 100, totalCost: Math.round(totalCost * 100) / 100 }; })()
  }

  function updateKPIs(result) {
    document.getElementById('kpi_downPaymentAmount').textContent = fmtCurrency(result.downPaymentAmount);
